...
void meshRender(
	    ...
	    if (sha->attrDim != mesh->attrDim)
            printf("Error: attrDim's do not match");
        else {
            int attrDim = sha->attrDim;
            int varyDim = sha->varyDim;
            int i, *tri;
            double aa[attrDim], bb[attrDim], cc[attrDim];
            double a[varyDim], b[varyDim], c[varyDim];

            /* For each triangle */
            for (i = 0; i < mesh->triNum; i++) {
                tri = meshGetTrianglePointer(mesh, i);

                /* First assigns each vertex to attr */
                vecCopy(attrDim, meshGetVertexPointer(mesh, tri[0]), aa);
                vecCopy(attrDim, meshGetVertexPointer(mesh, tri[1]), bb);
                vecCopy(attrDim, meshGetVertexPointer(mesh, tri[2]), cc);

                /* Transforms each vertex to be of vary form */
                sha->shadeVertex(sha->unifDim, unif, attrDim, aa, varyDim, a);
                sha->shadeVertex(sha->unifDim, unif, attrDim, bb, varyDim, b);
                sha->shadeVertex(sha->unifDim, unif, attrDim, cc, varyDim, c);

                /*  Renders triangle abc */
