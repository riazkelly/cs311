...
void texSample(const texTexture *tex, double s, double t, double sample[]) {
    ...
    if (tex->filtering == texNEAREST)
        ...
        int dimen = tex->texelDim;
        double fracU = u - floor(u), fracV = v - floor(v), temp1[dimen], temp2[dimen], temp3[dimen];
        texGetTexel(tex, (int)floor(u), (int)floor(v), sample);
        vecScale(dimen, (1-fracU)*(1-fracV), sample, sample);
        texGetTexel(tex, (int)ceil(u), (int)floor(v), temp1);
        vecScale(dimen, (fracU)*(1-fracV), temp1, temp1);
        texGetTexel(tex, (int)floor(u), (int)ceil(v), temp2);
        vecScale(dimen, (1-fracU)*(fracV), temp2, temp2);
        texGetTexel(tex, (int)ceil(u), (int)ceil(v), temp3);
        vecScale(dimen, (fracU)*(fracV), temp3, temp3);
        vecAdd(dimen, sample, temp1, sample);
        vecAdd(dimen, sample, temp2, sample);
        vecAdd(dimen, sample, temp3, sample);
    }
}
