...
void meshRender(
	    ...
	    if (sha->attrDim != mesh->attrDim)
            ...
            int i, *tri;
            double a[attrDim], b[attrDim], c[attrDim];
            ...
            for (i = 0; i < mesh->triNum; i++) {
                ...
                vecCopy(attrDim, meshGetVertexPointer(mesh, tri[0]), a);
                vecCopy(attrDim, meshGetVertexPointer(mesh, tri[1]), b);
                vecCopy(attrDim, meshGetVertexPointer(mesh, tri[2]), c);
