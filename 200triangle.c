...
void interpolate(int i, int j, const double unif[], const texTexture *tex[], const shaShading *sha, const double a[], const double b[], const double c[], double chi[]) {
    /* This functions interpolates a, b, and c to an varyDim–dimensional vector chi.
        varyDim is passed to us through shaShading. */
    ...
    double AInv[2][2], bMinusA[varyDim], cMinusA[varyDim], pq[2], xMinusa[2], X[2];
    double det;
    double vary[varyDim];
    double rgb[3];
    ...
    if (det != 0.0) {
        ...
        vecScale(varyDim, pq[0], bMinusA, bMinusA);
        vecScale(varyDim, pq[1], cMinusA, cMinusA);
        vecAdd(varyDim, a, bMinusA, vary);
        vecAdd(varyDim, vary, cMinusA, vary);

        shadeFragment(sha->unifDim, unif, sha->texNum, tex, sha->varyDim, vary, rgb);
